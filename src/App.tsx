  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#0a0f1e] text-slate-100 flex flex-col relative">
      ...
      <div className="relative z-10 flex-1 min-h-0 flex flex-col w-full max-w-[920px] mx-auto overflow-hidden">
        <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-4 sm:px-6 py-8 space-y-5 chat-scroll scrollbar-none">
          {!hasStarted ? (
            ...
            <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-5">
              {messages.map(m => (
                <motion.div key={m.id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={spring} className={cn("flex", m.role === 'user' ? 'justify-end' : 'justify-start')}>
                  {m.role === 'user' ? (
